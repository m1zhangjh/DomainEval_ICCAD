...
\newtext{
Reconfigurable computing, such as \cite{wildermann2011operational}, aims to support multiple applications (one app at a time) using FPGA, however, its efficiency relies on functional and structural similarities across reconfiguration cycles.
}
...
%Design Automation
\newtext{
Current design automation \cite{SCE}\cite{Daedalus}\cite{gruttner2011challenges} mainly contains two steps: platform architecture allocation and application-specific mapping (included binding and scheduling).
There is much research about mapping different applications on the existing architecture\cite{marwedel2011mapping}\cite{quan2013scenario}. However, platform allocation for many applications is less touched. 
Some ideas about how to consider many applications can be extracted from platform-based design\cite{PBD}\cite{lukasiewycz2009combined}. It uses statistical information to design the platform interconnections for many applications. However, its ACC allocation is either from engineering experiences or application-specific \cite{PBDmapping}\cite{gladigau2010system}, which requires large NRE.
}


%algorithm
\newtext{
Design algorithm is essential in automation, which are widely used to deal with the vast and complex design space.
...
These existing algorithms have been applied on allocating/mapping a platform for a single application, while unified platform allocation for many applications is less touched. The reason is that there is little existing platform evaluation that considers many applications to guide these algorithms.
}
